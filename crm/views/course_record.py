...
class CourseRecordHandler(PermissionHandler, StarkHandler):
    ...
    def display_homework(self, obj=None, is_header=None, *args, **kwargs):
        if is_header:
            return '作业'
        record_url = reverse('stark:crm_homework_list', kwargs={'course_record_id': obj.pk})
        return mark_safe('<a target="_blank" href="%s">作业</a>' % record_url)

    list_display = [StarkHandler.display_checkbox, 'class_object', 'day_num', 'teacher',
                    get_datetime_text('时间', 'date'), display_attendance,display_homework]
