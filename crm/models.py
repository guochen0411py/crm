...
class ClassList(models.Model):
    ...
    tech_teachers = models.ManyToManyField(verbose_name='任课老师', to='UserInfo', related_name='teach_classes', blank=True,
                                           limit_choices_to={'depart__title__in': ['教质部']})
...
class HomeWork(models.Model):
    ...
    作业表
    """
    title = models.CharField(verbose_name="作业名称",max_length=32)
    course_record = models.ForeignKey(verbose_name="课程记录",to="CourseRecord",on_delete=models.CASCADE)
    create_date = models.DateTimeField(verbose_name="创建时间",auto_now_add=True)
    hand_in_date = models.DateTimeField(verbose_name="最晚上交时间")
...
class HomeWorkRecord(models.Model):
    ...
    作业记录表
    """
    homework = models.ForeignKey(verbose_name="作业名称",to="HomeWork",on_delete=models.CASCADE)
    status_choices = [
        (1,"待批阅"),
        (2, "已批阅"),
    ]
    ...
    content = models.TextField(verbose_name="作业内容")  # 搞半天没搞出文件上传咋弄，基于ModelForm的
    create_date = models.DateTimeField(verbose_name="提交时间",auto_now_add=True)
    teacher = models.ForeignKey(verbose_name="批改老师",to="UserInfo",on_delete=models.CASCADE,limit_choices_to={'depart__title': '教质部'})
    score = models.IntegerField(verbose_name="作业成绩",default=0)
