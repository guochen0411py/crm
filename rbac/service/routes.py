    ...
    for item in urlpatterns:
        if isinstance(item, URLPattern):  # 非路由分发，讲路由添加到url_ordered_dict
            ...
            if pre_namespace:
                ...
                name = item.name
            url = pre_url + item.pattern.describe().split(" ")[0]
            ...
            if pre_namespace:
                ...
                if item.namespace:
                    ...
                    namespace = None
            recursion_urls(namespace, (pre_url+item.pattern.describe()).replace('^', '').replace('$', '').replace("'","") , item.url_patterns, url_ordered_dict)
